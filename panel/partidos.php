...
if (isset($_POST['grabar'])) {
	if (isset($_POST['accion'])) {
		if ($_POST['accion'] == "insertar") $sql = "insert into partidos values (".$_POST['idzonam'].",".$_POST['nrofecham'].",".$_POST['npartido'].",".$_POST['idarbitro'].",".$_POST['local'].",".$_POST['visitante'].",'".$_POST['fecha']."','".$_POST['hora']."'".")";
	}elseif ($_POST['accion'] == "modificar") $sql = "UPDATE partidos SET idpartido = '" . $_POST['npartido'] . "',local = '" . $_POST['local'] . "',visitante = '" . $_POST['visitante'] . "',fecha = '" . $_POST['fecha'] . "',hora = '" . $_POST['hora'] . "' WHERE idpartido = " . $_POST['idpartido'] . " and idzona=" . $_POST['idzona'] . " and nrofecha=" . $_POST['nrofecha'];
...
if (isset($_GET['modificar'])) {
	$sql = "SELECT p.idzona, z.zona, p.nrofecha, p.idpartido, p.npartido, p.local, e.equipo elocal, p.visitante, c.equipo evisitante, p.fecha , p.hora FROM arbitros a, zonas z, partidos p LEFT JOIN equipos e ON (e.idequipo = p.local), partidos h LEFT JOIN equipos c ON (c.idequipo = h.visitante) WHERE z.idzona = p.idzona and p.idpartido = " . $_GET['idpartido'] . " and p.idzona=" . $_GET['idzona'] . " and p.nrofecha=" . $_GET['nrofecha'];
	$resultado = $con->query($sql);
	$fila = mysqli_fetch_object($resultado);
	$npartido = $fila->npartdio;
	...
	if (isset($_POST['npartido'])) $npartido = $_POST['npartido'];
	elseif (isset($_GET['npartido'])) $npartido = $_GET['npartido'];
	else $npartido = "";
	if (isset($_POST['idpartido'])) $idpartido = $_POST['idpartido'];
	elseif (isset($_GET['idpartido'])) $idpartido = $_GET['idpartido'];
	else $idpartido = "";
	if (isset($_POST['idzona'])) $idzona = $_POST['idzona'];
	elseif (isset($_GET['idzona'])) $idzona = $_GET['idzona'];
	else $idzona = "";
	if (isset($_POST['nrofecha'])) $nrofecha = $_POST['nrofecha'];
	elseif (isset($_GET['nrofecha'])) $nrofecha = $_GET['nrofecha'];
	else $nrofecha = "";
	if (isset($_POST['fecha'])) $fecha = $_POST['fecha'];
	elseif (isset($_GET['fecha'])) $fecha = $_GET['fecha'];
	else $fecha = "";
	if (isset($_POST['hora'])) $hora = $_POST['hora'];
	elseif (isset($_GET['hora'])) $hora = $_GET['hora'];
	else $hora = "";
	if (!isset($_POST['local'])) $idequipolocal = "";
	else $idequipolocal = $_POST['local'];
	if (!isset($_POST['visitante'])) $idequipovisitante = "";
	else $idequipovisitante = $_POST['visitante'];
...
if (isset($_POST['publicar'])) {
	...
	for ($i = 1; $i <= 4; $i++) {
		$sql = "SELECT p.nrofecha, p.npartido, z.idzona, z.zona, p.local, p.fecha, p.hora, (SELECT equipo FROM equipos where idequipo = p.local) elocal, (SELECT equipo FROM equipos where idequipo = p.visitante) evisitante FROM partidos p, zonas z WHERE z.idzona = p.idzona and p.idzona=".$i." order by p.idzona, p.nrofecha";
		//$sql = "SELECT p.nrofecha, p.idpartido, npartido, z.idzona, z.zona, p.local, p.fecha, p.hora, (SELECT equipo FROM equipos where idequipo = p.local) elocal, (SELECT equipo FROM equipos where idequipo = p.visitante) evisitante FROM partidos p, zonas z WHERE z.idzona = p.idzona and p.idzona=" . $i . " and p.nrofecha = 4 order by p.idzona, p.nrofecha";
		$resultado = $con->query($sql);
		$count = mysqli_num_rows($resultado);
		...
		if ($count > 0) {
			...
			while ($fila = mysqli_fetch_object($resultado)) {
				...
				$tabla .= "<tbody><tr><td align='center'>" . $fila->npartido . "</td><td>" . $fila->elocal . "</td><td align='center'>Vs.</td><td>" . $fila->evisitante . "</td><td align='center'>" . substr($fila->hora, 0, 5) . "</td>
			    </tr></tbody>";
				$idzona = $fila->idzona;
	...
	<div id="section">
		...
		?>
			...
			<form name="abm" method="POST" action="<?php print $archivopost; ?>">
					...
					<input type="hidden" id="idzonam" name="idzonam" value="<?php print $idzona; ?>">
					<input type="hidden" id="idpartido" name="idpartido" value="<?php print $idpartido; ?>">
					<input type="hidden" id="nrofecham" name="nrofecham" value="<?php print $nrofecha; ?>">
					<label for="fecha">Fecha:;</label>
					<input type="text" id="fecha" name="fecha" value="<?php print $fecha; ?>">
					<br><br>
					<label for="hora">Hora:;</label>
					<input type="time" id="hora" name="hora" value="<?php print $hora; ?>">
					<BR><BR>
					<label for="npartido">Nro. Partido:</label>
					<input type="number" id="npartido" name="npartido" size="2" value="<?php print $npartido; ?>">
					<br><br>
					<label for="">Arbitro</label>
					<?php print $comboarbitro; ?>
					<BR><BR>
